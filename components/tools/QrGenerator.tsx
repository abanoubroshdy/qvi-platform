...
export function QrGenerator() {
  ...
  const generate = useCallback(async (value: string, pixelSize: number) => {
    ...
    if (content.length > MAX_CHARS) {
      setError(`النص أطول من الحد المسموح (${MAX_CHARS} حرفاً). اختصر المحتوى ثم أعد المحاولة.`);
      return;
    }
    ...
    try {
      ...
      setDataUrl(null);
      setBlob(null);
      setError("تعذر إنشاء رمز QR. جرّب نصاً أقصر أو أزل الرموز غير المدعومة.");
    } finally {
  ...
  return (
    <ToolLayout
      hideDropzone
      emptyPreviewText="أدخل رابطاً أو نصاً في الحقل أعلاه ليظهر رمز QR هنا."
      actionLabel="إنشاء الرمز"
      onAction={() => generate(text, size)}
      actionDisabled={!text.trim() || text.trim().length > MAX_CHARS}
      actionLoading={isGenerating}
      downloadLabel="تنزيل PNG"
      onDownload={() => {
        if (!blob) return;
      ...
      leading={
        <div className="rounded-xl border bg-card p-4 shadow-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {text.trim().length} / {MAX_CHARS} حرفاً · يُنشأ الرمز على جهازك فقط ولا يُرسل المحتوى لأي خادم.
          </p>
        </div>
      }
      extra={
        <div className="rounded-xl border bg-card p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="text-sm font-semibold tabular-nums text-primary">{size} بكسل</span>
          </div>
          <div dir="ltr">
            <Slider
              id="qr-size"
        ...
        dataUrl ? (
          <div className="grid gap-4 md:grid-cols-[auto_1fr] md:items-start">
            ...
            <div className="grid grid-cols-2 content-start gap-3">
              <Stat label="الحجم" value={`${size}×${size}`} />
              <Stat label="حجم الملف" value={blob ? formatBytes(blob.size) : "—"} />
              <Stat label="التصحيح" value="متوسط (M)" />
              <Stat label="الحالة" value={isGenerating ? "جارٍ الإنشاء" : "جاهز للتنزيل"} />
            </div>
          </div>
        ) : undefined
      }
